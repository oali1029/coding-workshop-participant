locals {
  ...
  env_vars = {
    ...
    # Empty by default; the lambda module drops empty values, so the backend
    # then skips creating the bootstrap administrator instead of failing.
    BOOTSTRAP_ADMIN_PASSWORD = try(trimspace(var.aws_bootstrap_admin_password), "")
